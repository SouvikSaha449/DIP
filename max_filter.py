...
for i in range(1, y - 1):
    for j in range(1, x - 1):
        img2[i, j] = max(img1[i - 1, j - 1], img1[i, j - 1], img1[i + 1, j -1], img1[i - 1, j], img1[i, j], img1[i + 1, j], img1[i - 1, j + 1], img1[i, j + 1], img1[i + 1, j + 1])

with open ('images/3_max.pgm', 'w') as img:
    img.write(word)
    img.write(comment)
    img.write(res)
